...
function deleteUser(id) {
  mergedUser = mergedUser.filter((data) => data.id != id);
  renderUser();
}
...
function renderUser() {
  ...
  for (let i = mergedUser.length - 1; i >= 0; i--) {
    ...
    btn.onclick = () => deleteUser(mergedUser[i].id);
    li.appendChild(btn);
...
function createClassObject() {
  ...
  mergedUser.forEach(({ id, name, email, age }) => {
    userObj.push(new User(id, name, email, age));
  });
